    ...
    return {
      ...
      permissions: [
        ...
        },
        {
          id: 'stone_create',
          title: '스톤생성',
          description: '새로운 스톤을 생성할 수 있는 권한을 허용합니다. 본인이 생성한 스톤에 대해서 관리할 수 있습니다.',
          enabled: false
        },
        {
          id: 'user_group_add',
          title: '사용자 그룹 추가',
          description: '새로운 사용자 그룹을 추가할 수 있는 권한을 허용합니다.',
          enabled: false
        },
        {
          id: 'sun_file_view',
          title: '태양별 파일 조회',
          description: '태양별로 파일을 조회할 수 있는 권한을 허용합니다.',
          enabled: false
        }
      ]
    };
  ...
  methods: {
    ...
    async createPermissionGroup() {
      ...
      try {
        ...
        // AccessType.values() 순서: PROJECT_CREATE, MEMBER_INVITE, STONE_FILE_VIEW, STONE_CREATE, USER_GROUP_ADD, SUN_FILE_VIEW, ADMIN
        const accessList = [
          this.permissions.find(p => p.id === 'project_create')?.enabled || false,
          this.permissions.find(p => p.id === 'member_invite')?.enabled || false,
          this.permissions.find(p => p.id === 'stone_file_view')?.enabled || false,
          this.permissions.find(p => p.id === 'stone_create')?.enabled || false,
          this.permissions.find(p => p.id === 'user_group_add')?.enabled || false,
          this.permissions.find(p => p.id === 'sun_file_view')?.enabled || false,
          false // ADMIN 권한은 별도로 관리
        ];
